 ...
 * - archivar (default): activo=false (seguro)
 * - eliminar: requiere ALLOW_HARD_DELETE=1 (para evitar accidentes)
 */
export const eliminarPaciente = async (req, res) => {
  try {
    ...
    if (idsTratamientos.length) {
      await Promise.all([
        Pago.deleteMany({ tratamientoId: { $in: idsTratamientos } }),
        Gasto.deleteMany({ tratamientoId: { $in: idsTratamientos } }),
        Tratamiento.deleteMany({ _id: { $in: idsTratamientos } }),
      ]);
    }
    ...
    return res.json({
      ok: true,
      modo,
      message: "Paciente eliminado definitivamente",
